...
Follow this step-by-step master checklist to deploy **DevOps PDFMasterPro** from scratch to production.
...
- [ ] **Step 6**: Log in to the Azure CLI and select the correct subscription:
  ...
  ```
- [ ] **Step 7**: Create the Resource Group `rg-devops-pdfmasterpro-prod`.
- [ ] **Step 8**: Create the Azure Key Vault `kv-devops-pdfmasterpro-prod-01` with RBAC authorization enabled.
- [ ] **Step 9**: Create the User Assigned Managed Identity (UAMI) `func-devops-pdfmasterpro-prod-uami`.
...
- [ ] **Step 13**: Create the Linux Node.js Azure Function App `func-devops-pdfmasterpro-prod`.
- [ ] **Step 14**: Attach the Managed Identity `func-devops-pdfmasterpro-prod-uami` to the Function App.
...
- [ ] **Step 19**: Create the Static Web App `swa-devops-pdfmasterpro-prod` and retrieve the API token.
...
- [ ] **Step 23**: Configure the frontend environment variable in your Static Web App setting in the Azure portal (under Configuration):
  - `VITE_API_BASE_URL` -> `https://func-devops-pdfmasterpro-prod.azurewebsites.net`
